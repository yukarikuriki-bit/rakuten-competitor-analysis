 import streamlit as st
  ...
  def search_rakuten(keyword, num_results=20):
      ...
      try:
              ...
              try:
                  ...
                  if data.get("@type") == "ItemList":
                      for el in data.get("itemListElement", []):
                          ...
                          if name and item_url and "item.rakuten.co.jp" in item_url:
                              products.append({
                                  "name": name,
          ...
          # ── 方法2: aタグから直接取得（フォールバック）──
          if not products:
              seen = set()
              for a in soup.find_all("a", href=True):
                  href = a["href"]
                  if "item.rakuten.co.jp" in href and href not in seen:
                      seen.add(href)
                      products.append({
                          "name": a.get_text(strip=True) or "（商品名取得不可）",
                          "url": href.split("?")[0],
                      })
                  if len(products) >= num_results:
                      break
  ...
  if run:
      ...
      for i, product in enumerate(products):
          progress_bar.progress((i + 1) / len(products), text=f"確認中 ({i+1}/{len(products)}):
  {product['name'][:30]}...")
              ...
              for _, row in matched_df.iterrows():
                  with st.container():
                      st.markdown(
                          ...
                          f"該当レビュー数: **{int(row[keyword_col])}件** ／ 取得レビュー総数:
  {int(row['レビュー取得数'])}件"
                      )
